...
$(document).on('click', '.edit-btn', function () {
   ...
   $.ajax({
    url: baseUrl + 'sales/edit/' + userId,
    method: 'GET',
    dataType: 'json',
    success: function (response) {
        if (response.data) {
            $('#editUserModal #sale_date').val(response.data.sale_date);
            $('#editUserModal #userId').val(response.data.id);
            $('#editUserModal #total_amount').val(response.data.total_amount);
...
$(document).on('click', '.deleteUserBtn', function () {
    ...
    if (confirm('Are you sure you want to delete this user?')) {
        $.ajax({
            url: baseUrl + 'sales/delete/' + userId,
            method: 'POST', 
            data: {
                _method: 'DELETE',
                [csrfName]: csrfToken
